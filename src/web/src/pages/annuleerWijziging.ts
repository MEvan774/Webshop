    ...
    bevestigKnop.addEventListener("click", async () => {
        if (!token) {
            ...
            if (typeof tokenData === "boolean") {
                ...
                statusEl.innerHTML = "Wacht op annulering";
                await cancelEmailChange(tokenData.email, tokenData.userId);

                bevestigDiv.innerHTML = "";
                statusEl.innerHTML =
                "De wijziging is geannuleerd! Klik <a href='/login.html'>hier</a> om in te loggen.";
            }
        }
    });
}
...
/**
 * Send a request to change the email back and delete all tokens
 *
 * @param email Email of be changed back
