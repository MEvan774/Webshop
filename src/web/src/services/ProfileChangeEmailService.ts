...
export class ProfileChangeEmailService implements IProfileChangeEmailService {
    ...
    public async writeEmail(kind: string, name: string, email: string, userId: number, newEmail?: string): Promise<boolean> {
        ...
        if (kind === "new") {
            const link: string = `https://naagooxeekuu77-pb4sef2425.hbo-ict.cloud/bevestigWijziging.html?token=${token}`;
            ...
            const link: string = `https://naagooxeekuu77-pb4sef2425.hbo-ict.cloud/annuleerWijziging.html?token=${token}`;
